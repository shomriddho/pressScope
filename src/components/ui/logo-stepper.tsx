...
export const LogoStepper: React.FC<LogoStepperProps> = ({
  ...
  const getVisibleLogos = () => {
    ...
    for (let i = -offset; i <= offset; i++) {
      ...
      visible.push({ logo: logos[index], originalIndex: index, position: i })
    }
    return visible
  }
          ...
          {visibleLogos.map(({ logo, originalIndex, position }) => {
            ...
            return (
              <motion.div
                key={originalIndex}
                layout="position"
                initial={{ x: direction === 'loop' ? 120 : -120, opacity: 0, scale: 0.7 }}
                animate={{
                  x: 0,
                  opacity: position === 0 ? 1 : 0.3,
                  scale: position === 0 ? 1 : 0.75,
                }}
                exit={{ x: direction === 'loop' ? -120 : 120, opacity: 0, scale: 0.7 }}
